						...
						<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
							<div class="card-header align-items-center  border-bottom-dark px-0">
							    ...
							    <div class="icons d-flex">
									<a href="{{ url('gate-serialnumber/form/-') }}" class="btn btn-outline-primary rounded-pill font-weight-bold me-1 mb-1">Tambah Data</a>
								</div>
							</div>
						</div>
					</div>
													...
													<td>
														<div class="card-toolbar text-end">
															...
															<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdowneditButton1">
																<a class="dropdown-item" href="{{ url('gate-serialnumber/form/' . $v->id) }}">Edit</a>
                                                                <form action="{{ url('gate-serialnumber/delete/' . $v->id) }}" method="POST" style="display:inline;">
                                                                    @csrf
                                                                    @method('DELETE')
...
<script type="text/javascript">
    ...
    function submitBlock() {
        ...
        $.ajax({
            url: "{{ route('gate-serialnumber-block') }}",
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                SerialNumber: sn,
    ...
    function unblockSN(sn) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('gate-serialnumber-unblock') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        SerialNumber: sn
