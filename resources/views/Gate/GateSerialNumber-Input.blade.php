		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-white mb-0 px-0 py-2">
				<li class="breadcrumb-item active" aria-current="page">
					<a href="{{url('gate-serialnumber')}}">Serial Number</a>
				</li>
				...
				<div class="row">
					<div class="col-12  px-4">
						<div class="card card-custom gutter-b bg-white border-0" >
							<div class="card-body" >
								@if ($serialNumber)
                            		<form action="{{url('gate-serialnumber/edit')}}" method="post">
                                        <input type="hidden" name="id" value="{{ $serialNumber->id }}">
                            	@else
                                	<form action="{{url('gate-serialnumber/store')}}" method="post">
                            	@endif
                            		@csrf
	                            	<div class="form-group row">
	                            		...
	                            		<div class="col-md-12 mt-4">
                                            ...
                                            <a href="{{ url('gate-serialnumber') }}" class="btn btn-danger font-weight-bold me-1 mb-1">Batal</a>
	                            		</div>
	                            	</div>
                            	</form>
							</div>
...
<script type="text/javascript">
    ...
    function generateSerialNumber() {
        jQuery.ajax({
            url: "{{ url('gate-serialnumber/generate') }}",
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
