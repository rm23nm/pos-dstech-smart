                                    ...
                                    <table class="table table-bordered table-hover" id="tablePasien">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $p->NIK }}</td>
                                                <td>{{ $p->NamaPasien }}</td>
                                                <td>{{ $p->TanggalLahir ? date('d-m-Y', strtotime($p->TanggalLahir)) : '-' }}</td>
                                                <td>{{ $p->JenisKelamin == 'L' ? 'Laki-Laki' : ($p->JenisKelamin == 'P' ? 'Perempuan' : '-') }}</td>
                                                <td>{{ $p->NoHP }}</td>
                                                <td>{{ $p->GolonganDarah }}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-warning" onclick="editPasien({{ json_encode($p) }})"><i class="fas fa-edit"></i></button>
                                                    <a href="{{ route('klinik-patients.destroy', $p->id) }}" class="btn btn-sm btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i></a>
                                                </td>
                                            </tr>
                                            @endforeach
                                        </tbody>
...
<div class="modal fade" id="modalPasien" tabindex="-1" aria-labelledby="modalPasienLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="formPasien" method="POST" action="{{ route('klinik-patients.store') }}">
                @csrf
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="ProviderAsuransiLain" id="ProviderAsuransiLain" class="form-control">
                                ...
                                @foreach($asuransis as $asuransi)
                                    <option value="{{ $asuransi->NamaAsuransi }}">{{ $asuransi->NamaAsuransi }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
...
<script>
    ...
    function tambahPasien() {
        ...
        $('#formPasien').attr('action', '{{ route('klinik-patients.store') }}');
        $('#NIK').val('');
        $('#NamaPasien').val('');
        $('#TanggalLahir').val('');
        $('#JenisKelamin').val('');
    ...
    function editPasien(data) {
        ...
        $('#formPasien').attr('action', '{{ url('klinik-patients/update') }}/' + data.id);
