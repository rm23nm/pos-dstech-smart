...
<body>
    ...
    <!-- HEADER -->
    <div class="header-tv">
        <h1><i class="fas fa-plus-square"></i> KLINIK INDONESIA - {{ strtoupper($poli->NamaPoli) }}</h1>
            ...
            <div class="current-call-box">
                <div class="doctor-info-header">
                    ...
                    <div class="doctor-details">
                        <h2 id="currentPoli">POLI {{ strtoupper($poli->NamaPoli) }}</h2>
                        @if(!empty($poli->Shift))
                            <div class="text-warning font-weight-bold" style="font-size: 1.2rem; margin-bottom: 5px;"><i class="fas fa-clock"></i> {{ $poli->Shift }}</div>
                        @endif
                        <h3 id="currentDoctor">{{ $defaultDoctor ?? 'Menunggu Panggilan' }}</h3>
                    </div>
                </div>
        ...
        <div class="right-panel">
            <div class="video-container">
                ...
                @if(count($playlist) > 0)
                    ...
                    <script>
                        var mediaPlaylist = @json($playlist);
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function fetchDisplayData() {
                $.ajax({
                    url:  "{{ route('klinik-display-poli.data', ['poli_id' => $poli->id]) }}",
                    type: 'GET',
                    success: function(res) {
                        ...
                        if (current !== '---' && (current !== lastCalled || updatedAt !== lastUpdatedAt)) {
                            ...
                            $('#currentPoli').text('POLI {{ strtoupper($poli->NamaPoli) }}');
                            $('#currentDoctor').text('{{ $defaultDoctor ?? "Menunggu Panggilan" }}');
