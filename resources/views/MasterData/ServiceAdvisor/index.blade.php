        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                ...
                <div class="card-toolbar">
                    <a href="{{ route('fpenjualan-custdisplay') }}" target="_blank" class="btn btn-info text-white me-2">
                                ...
                                <div class="input-group">
                                    <select class="form-control select2" name="PlatNomor" id="PlatNomor" style="width: 85%;" required>
                                        ...
                                        @foreach($kendaraans as $k)
                                            <option value="{{ $k->PlatNomor }}" data-pelanggan="{{ $k->KodePelanggan }}">{{ $k->PlatNomor }} ({{ $k->Merek }} {{ $k->Tipe }})</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12 mb-2">
                                    ...
                                    <select class="form-control select2" id="tempKategori" style="width:100%">
                                        ...
                                        @foreach($kategoris as $kat)
                                            <option value="{{ $kat->KodeJenis }}">{{ $kat->NamaJenis }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
...
<script>
    ...
    jQuery(document).ready(function() {
        ...
        // Fetch Items
        jQuery.ajax({
            url: "{{ route('service-advisor.items') }}",
            type: "GET",
        ...
        table = jQuery('#dataTable').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: "{{ route('service-advisor.getData') }}",
                type: "POST",
                data: function (d) {
                    d._token = "{{ csrf_token() }}";
                }
    ...
    function savePKB() {
        ...
        jQuery.ajax({
            url: "{{ route('service-advisor.store') }}",
            type: 'POST',
            data: payload,
    ...
    function submitKendaraan() {
        ...
        jQuery.ajax({
            url: "{{ route('service-advisor.kendaraan.store') }}",
            type: "POST",
    ...
    function submitPelanggan() {
        ...
        jQuery.ajax({
            url: "{{ route('service-advisor.pelanggan.store') }}",
            type: "POST",
    ...
    function batalServis(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                jQuery.ajax({
                    url: "{{ url('service-advisor/batal') }}/" + id,
                    type: 'POST',
    ...
    function loadDetail(noPkb) {
        jQuery.ajax({
            url: "{{ url('service-advisor/details') }}/" + noPkb,
            type: "GET",
            success: function(res) {
                var tbody = '';
                var total = 0;
    ...
    function addDetail() {
        ...
        jQuery.ajax({
            url: "{{ route('service-advisor.details.store') }}",
            type: "POST",
            data: formData,
            success: function(res) {
                if(res.success) {
    ...
    function deleteDetail(id, noPkb) {
        jQuery.ajax({
            url: "{{ url('service-advisor/details/delete') }}/" + id,
            type: "POST",
            data: { _token: "{{ csrf_token() }}" },
            success: function(res) {
                if(res.success) {
