...
class TableOrderController extends Controller
{
    ...
    public function ViewNew(Request $request)
    {
        ...
        $subqueryPembayaranJasa = FakturPenjualanDetail::selectRaw("fakturpenjualandetail.BaseReff, fakturpenjualandetail.RecordOwnerID,
                            ...
                            ->join('itemmaster', function($join) use ($roid) {
                                $join->on('fakturpenjualandetail.KodeItem', '=', 'itemmaster.KodeItem')
                                    ->on('itemmaster.RecordOwnerID', '=', 'fakturpenjualandetail.RecordOwnerID')
        ...
        $titiklampu = TitikLampu::selectRaw("titiklampu.*,
                            CASE 
                                ...
                                WHEN titiklampu.Status = 1 THEN 'AKTIF' 
                                WHEN titiklampu.Status = -1 THEN 'CHECKOUT' 
                                WHEN titiklampu.Status = 99 THEN 'HAMPIR HABIS' 
                                ELSE 'KOSONG' 
                            END StatusMeja,
                            COALESCE(tableorderheader.NoTransaksi,'') AS NoTransaksi,
                            tableorderheader.TglPencatatan,
        ...
        $gudangPoS = (count($company) > 0 && $company[0]->GudangPoS) ? $company[0]->GudangPoS : 'GDG01';
        ...
        $itemmaster = ItemMaster::selectRaw("itemmaster.KodeItem, itemmaster.NamaItem, itemmaster.HargaJual, itemmaster.Gambar, itemmaster.TypeItem, COALESCE(itemwarehouses.Qty, 0) as Stock")
                    ->leftJoin('itemwarehouses', function($join) use ($roid, $gudangPoS) {
                        $join->on('itemmaster.KodeItem', '=', 'itemwarehouses.KodeItem')
                            ...
                            ->where('itemwarehouses.KodeGudang', '=', $gudangPoS);
                    })
                    ->where('itemmaster.RecordOwnerID', $roid)
        ...
        $kelompoklampu = KelompokLampu::where('RecordOwnerID','=',$roid)->get();
        $gruppelanggan = GrupPelanggan::where('RecordOwnerID','=',$roid)->get();
        $jenisLangganan = [];
        if (count($company) > 0 && $company[0]->JenisLangganan) {
            $jenisLangganan = json_decode($company[0]->JenisLangganan, true);
        }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            // COLLISION DETECTION: Check for overlapping transactions on this table
            $checkEnd = $model->JamSelesai ?? (clone $model->JamMulai)->addHours(24);
            $overlap = DB::table('tableorderheader')
                ->where('tableid', $model->tableid)
                ->where('RecordOwnerID', $roid)
                ->whereIn('DocumentStatus', ['O', 'D'])
                ->where(function($q) use ($model, $checkEnd) {
                    $q->where('JamMulai', '<', $checkEnd)
                      ->where(DB::raw("COALESCE(JamSelesai, DATE_ADD(JamMulai, INTERVAL 24 HOUR))"), '>', $model->JamMulai);
                })
                ->exists();
        ...
        try {
            ...
            if ($currentData) {
                ...
                if ($validOrder) {
                    ...
                    // SINKRONISASI LAMPU: Aktifkan kembali
                    DB::table('titiklampu')
                        ->where('id', $currentData->tableid)
                        ->where('RecordOwnerID', Auth::user()->RecordOwnerID)
                        ->update(['Status' => 1]);
    ...
    {
        $roid = Auth::user()->RecordOwnerID;
        Log::info("DEBUG storePaket REQUEST: " . json_encode($request->all()));
        ...
        try {
            ...
            Log::error("storePaket VALIDATION FAILED: " . json_encode($e->errors()) . " | Request: " . json_encode($request->all()));
            return response()->json(['success' => false, 'message' => 'Validasi gagal: ' . implode(", ", array_keys($e->errors()))]);
        }
        ...
        try {
            ...
            // COLLISION DETECTION: Check for overlapping transactions on this table
            if ($model->JamMulai && $model->JamSelesai) {
                $overlap = DB::table('tableorderheader')
                    ->where('tableid', $model->tableid)
                    ->where('RecordOwnerID', Auth::user()->RecordOwnerID)
                    ->whereIn('DocumentStatus', ['O', 'D'])
                    ->where(function($q) use ($model) {
                        $q->where(function($sub) use ($model) {
                            $sub->where('JamMulai', '<', $model->JamSelesai)
                                ->where(function($inner) use ($model) {
                                    $inner->where('JamSelesai', '>', $model->JamMulai)
                                          ->orWhereNull('JamSelesai');
                                });
                        });
                    })
            ...
            foreach ($fnbItems as $item) {
                ...
                if ($allowNegative !== 'Y') {
                    ...
                    if ($currentStock < $qty) {
                        ...
                        return response()->json(['success' => false, 'message' => "Stok item $itemName ($kodeItem) tidak mencukupi (Tersedia: $currentStock, Diminta: $qty)."]);
                    }
                }
            ...
            if ($opsiBayar === 'LANGSUNG') {
                if ($mp) {
                    $gtBeforeAdmin = $dpp + $model->TotalTax + $model->TotalPajakHiburan + $model->BiayaLayanan + $totalMakanan + $totalTaxFnb + $totalServiceFnb;
                ...
                $model->TotalTerbayar = $dpp + $model->TotalTax + $model->TotalPajakHiburan + $model->BiayaLayanan + $totalMakanan + $totalTaxFnb + $totalServiceFnb + $adminFeeRp;
            } else {
                $model->TotalTerbayar = 0;
                $model->MetodePembayaran = null;
            }
            
            $model->TotalTax += $totalTaxFnb;
            $model->BiayaLayanan += $totalServiceFnb;
            $model->NetTotal = $dpp + $model->TotalTax + $model->TotalPajakHiburan + $model->BiayaLayanan + $totalMakanan;
...
public function getTableStatuses()
        ...
        try {
            ...
            $titiklampu = DB::table('titiklampu')
                ->selectRaw("
                    ...
                    END StatusMeja,
                    COALESCE(tableorderheader.NoTransaksi,'') AS NoTransaksi,
                    COALESCE(tableorderheader.paketid,0) as paketid,
                    COALESCE(pakettransaksi.NamaPaket,'') as NamaPaket,
                    tableorderheader.JamMulai,
                    tableorderheader.JamSelesai,
                    tableorderheader.JenisPaket,
                ...
                ->leftjoin('tkelompoklampu', function ($join) use ($roid) {
                    $join->on('titiklampu.KelompokLampu','=','tkelompoklampu.KodeKelompok')
                         ->on('titiklampu.RecordOwnerID','=','tkelompoklampu.RecordOwnerID')
                         ->where('tkelompoklampu.RecordOwnerID', '=', $roid);
                })
                ->where('titiklampu.RecordOwnerID', '=', $roid)
            ...
            Log::error("getTableStatuses Error: " . $th->getMessage());
            return response()->json([
                'success' => false,
    ...
    {
        $recordOwnerID = Auth::user()->RecordOwnerID;
        Log::info("DEBUG storeFnBOrder FULL REQUEST: " . json_encode($request->all()));
        ...
        try {
            ...
            if (empty($items)) {
                Log::error("storeFnBOrder FAILED: Items empty for NoTransaksi: $noTransaksi");
            ...
            foreach ($items as $index => $item) {
                ...
                if ($allowNegative !== 'Y') {
                    ...
                    if ($currentStock < $qty) {
                        DB::rollBack();
                        $errMsg = "Stok item $namaItem ($kodeItem) tidak mencukupi di gudang $gudangPoS (Tersedia: $currentStock, Diminta: $qty). Silahkan isi stok atau aktifkan 'Allow Negative Inventory' di pengaturan.";
                        Log::error("storeFnBOrder FAILED: $errMsg");
                        return response()->json([
                            'success' => false, 
                            'message' => $errMsg
                        ], 400);
        ...
        try {
            ...
            if ($paymentType === 'POS' || $paymentType === 'NEW_PACKAGE') {
                ...
                if ($model->JenisPaket !== 'PAKETMEMBER' || $model->NetTotal > 0) {
                    ...
                    // Finalize and add FnB items to invoice
                    $fnbOpen = DB::table('tableorderfnb')
                        ->where('NoTransaksi', $noTransaksi)
                        ->where('RecordOwnerID', $recordOwnerID)
                        ->where('LineStatus', 'O')
                    ...
                    foreach ($fnbOpen as $index => $item) {
                        $fdFnb = new FakturPenjualanDetail;
                        $fdFnb->NoTransaksi   = $invoiceNo;
                        $fdFnb->BaseReff      = $noTransaksi;
                        $fdFnb->NoUrut        = $index + 2;
                        $fdFnb->BaseLine      = 0;
                        $fdFnb->KodeItem      = $item->KodeItem;
                        $fdFnb->Qty           = $item->Qty;
                        $fdFnb->QtyKonversi   = $item->Qty;
                        $fdFnb->QtyRetur      = 0;
                        $fdFnb->Satuan         = "PCS";
                        $fdFnb->Harga          = $item->Harga;
                        $fdFnb->Discount       = $item->Discount;
                        $fdFnb->HargaNet       = $item->Qty * $item->Harga;
                        $fdFnb->LineStatus     = "C";
                        $fdFnb->KodeGudang     = $company->GudangPoS ?? 'HO';
                        $fdFnb->Keterangan     = "FnB - " . $noTransaksi;
                        $fdFnb->VatPercent     = $ppnPer;
                        $fdFnb->VatTotal       = $item->Tax;
                        $fdFnb->Pajak          = $item->Tax;
                        $fdFnb->PajakHiburan   = 0;
                        $fdFnb->RecordOwnerID  = $recordOwnerID;
                        $fdFnb->created_at     = Carbon::now();
                        $fdFnb->save();
