	...
	public function storePoSFnB(Request $request)
	{
		...
		try {
			...
			foreach ($jsonData['Detail'] as $key) {
				...
				// Cek apakah item ini adalah paket member FNB
				if (!empty($jsonData['KodePelanggan']) && $jsonData['KodePelanggan'] !== 'CASH') {
					$package = DB::table('member_packages')
						->where('RecordOwnerID', Auth::user()->RecordOwnerID)
						->where('KodePaket', $key['KodeItem'])
						->where('KategoriPaket', 'FNB')
						->first();
					...
					if ($package) {
						...
						if ($pelanggan) {
							$currentValidUntil = $pelanggan->ValidUntil ? Carbon::parse($pelanggan->ValidUntil, 'Asia/Jakarta') : Carbon::now('Asia/Jakarta');
							if ($currentValidUntil->isPast()) {
								$currentValidUntil = Carbon::now('Asia/Jakarta');
							}
...
$updateData = [
							...
							if (($package->DiskonPersen ?? 0) > 0) {
								$updateData['DiskonMemberPersen'] = $package->DiskonPersen;
							}
							if (($package->MaxGratisOngkir ?? 0) > 0) {
								$updateData['SisaGratisOngkir'] = DB::raw("COALESCE(SisaGratisOngkir, 0) + (" . $package->MaxGratisOngkir . " * " . $key['Qty'] . ")");
							}
			...
			if ($jsonData['Status'] != 'T' && $jsonData['KodePelanggan'] != "") {
				if (isset($jsonData['PoinDitukar']) && $jsonData['PoinDitukar'] > 0) {
					DB::table('pelanggan')
						->where('RecordOwnerID', Auth::user()->RecordOwnerID)
						->where('KodePelanggan', $jsonData['KodePelanggan'])
						->update([
							'PoinLoyalti' => DB::raw("COALESCE(PoinLoyalti, 0) - " . intval($jsonData['PoinDitukar']))
						]);
				}

				if (($oCompany->KonversiRupiahKePoin ?? 0) > 0) {
					$poinDidapat = floor($jsonData['TotalPembelian'] / $oCompany->KonversiRupiahKePoin);
					if ($poinDidapat > 0) {
						DB::table('pelanggan')
							->where('RecordOwnerID', Auth::user()->RecordOwnerID)
							->where('KodePelanggan', $jsonData['KodePelanggan'])
	...
	public function storePoSHiburan(Request $request) {
		...
		try {
			...
			foreach ($jsonData['Detail'] as $key) {
				...
				// Cek apakah item ini adalah paket member
				if (!empty($jsonData['KodePelanggan']) && $jsonData['KodePelanggan'] !== 'CASH') {
					$package = DB::table('member_packages')
						->where('RecordOwnerID', Auth::user()->RecordOwnerID)
						->where('KodePaket', $key['KodeItem'])
	...
	public function storePoS(Request $request)
	{
	  ...
	  //       			);
			...
			foreach ($jsonData['Detail'] as $key) {
				...
				// Cek apakah item ini adalah paket member RETAIL
				if (!empty($jsonData['KodePelanggan']) && $jsonData['KodePelanggan'] !== 'CASH') {
					$package = DB::table('member_packages')
						->where('RecordOwnerID', Auth::user()->RecordOwnerID)
						->where('KodePaket', $key['KodeItem'])
						->where('KategoriPaket', 'RETAIL')
						->first();
...
$updateData = [
	...
	public function storeJson(Request $request)
	{
		...
		try {
			...
			$model->NoResep = empty($jsonData['NoResep']) ? "" : $jsonData['NoResep'];
			$model->NamaDokter = empty($jsonData['NamaDokter']) ? "" : $jsonData['NamaDokter'];
			$model->NamaPasien = empty($jsonData['NamaPasien']) ? "" : $jsonData['NamaPasien'];
