...
<head>
    ...
    <!-- Styles -->
    <link href="<?php echo e(asset('css/style.css?v=1.0')); ?>" rel="stylesheet" type="text/css" />
    <link href="<?php echo e(asset('api/pace/pace-theme-flat-top.css')); ?>" rel="stylesheet" type="text/css" />
    <link href="<?php echo e(asset('api/select2/select2.min.css')); ?>" rel="stylesheet" />
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .left-panel {
            ...
            overflow-y: auto;
            padding: 24px;
            <?php if(count($company) > 0 && $company[0]->TypeBackgraund == 'Color' && !empty($company[0]->Backgraund)): ?>
                background-color: <?php echo e($company[0]->Backgraund); ?>;
            <?php elseif(count($company) > 0 && $company[0]->TypeBackgraund == 'Image' && !empty($company[0]->Backgraund)): ?>
                background: linear-gradient(rgba(244, 246, 251, 0.3), rgba(244, 246, 251, 0.3)), url('<?php echo e($company[0]->Backgraund); ?>') no-repeat center center;
...
<body>
    ...
    <header class="pos-header">
        <div class="brand d-flex align-items-center gap-2">
            <?php if(!empty($company[0]['icon'])): ?>
                <img src="<?php echo e($company[0]['icon']); ?>" alt="Logo" style="height: 40px; width: auto; border-radius: 4px; background: white; padding: 2px;">
            <?php else: ?>
                <img src="<?php echo e(asset('images/misc/LogoFront.png')); ?>" alt="Logo" style="height: 40px; width: auto;">
            <?php endif; ?>
            <span><?php echo e($company[0]['NamaPartner']); ?></span>
        </div>
        <div class="clock-display" id="posHeaderClock">--:--:--</div>
        <div class="d-flex align-items-center gap-3 header-actions">
            <a href="<?php echo e(route('ticketing-pos')); ?>" class="btn btn-sm btn-info text-white" style="background:#0ea5e9; border:none; padding:4px 10px; border-radius:4px;"><i class="fas fa-swimmer"></i> Ticketing / Kolam</a>
            ...
            <span class="user-info"><i class="fas fa-user-circle"></i> <?php echo e(Auth::user()->name); ?></span>
            ...
            <a href="<?php echo e(route('bookinglist')); ?>" target="_blank"><i class="fas fa-calendar-alt"></i> Booking</a>
    ...
    <div class="pos-main">
        ...
        <div class="left-panel">
            ...
            <?php if(count($kelompoklampu) > 0): ?>
                <?php $__currentLoopData = $kelompoklampu; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    ...
                    ?>
                    <?php if($itemInGroup->count() > 0): ?>
                        <div class="kelompok-section">
                            <div class="kelompok-title"><i class="fas fa-layer-group me-1"></i> <?php echo e($tl->NamaKelompok); ?></div>
                            <div class="titik-grid">
                                <?php $__currentLoopData = $itemInGroup; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                    ...
                                    ?>
                                    <div class="titik-box <?php echo e($statusClass); ?>"
                                         data-id="<?php echo e($item->id); ?>"
                                         data-namatitiklampu="<?php echo e($item->NamaTitikLampu); ?>"
                                         data-notransaksi="<?php echo e($item->NoTransaksi); ?>"
                                         data-jenispaket="<?php echo e($item->JenisPaket); ?>"
                                         data-status="<?php echo e($item->Status); ?>"
                                         data-namapaket="<?php echo e($item->NamaPaket ?? ''); ?>"
                                         data-jammulai="<?php echo e($item->JamMulai ? \Carbon\Carbon::parse($item->JamMulai)->format('d/m/Y H:i') : '-'); ?>"
                                         data-jamselesai="<?php echo e($item->JamSelesai ? \Carbon\Carbon::parse($item->JamSelesai)->format('d/m/Y H:i') : '-'); ?>"
                                         data-gambar="<?php echo e($item->Gambar ?? ''); ?>"
                                         data-statuslabel="<?php echo e($statusLabel); ?>"
                                         data-totalPembayaran="<?php echo e($item->TotalPembayaran ?? 0); ?>"
                                         data-rawjammulai="<?php echo e($item->JamMulai ?? ''); ?>"
                                         data-rawjamselesai="<?php echo e($item->JamSelesai ?? ''); ?>"
                                         data-namakelompok="<?php echo e($tl->NamaKelompok); ?>"
                                         data-kelompoklampu="<?php echo e($item->KelompokLampu ?? ''); ?>"
                                         onclick="selectTitikLampu(this)"
                                         title="<?php echo e($item->NamaTitikLampu); ?>"
                                         role="button">
                                        <div class="titik-nama"><?php echo e($item->NamaTitikLampu); ?></div>
                                        <?php if($item->Status != 0 && ($item->TotalPembayaran ?? 0) > 0): ?>
    ...
    <script src="<?php echo e(asset('devexpress/jquery.min.js')); ?>"></script>
    <script src="<?php echo e(asset('api/select2/select2.min.js')); ?>"></script>

    <script>
    // ===== DATA from PHP =====
    // All titik lampu data in JSON for JS use
    var titiklampuData = <?php echo json_encode($titiklampu, 15, 512) ?>;
    ...
    function fetchAndSyncCustomerDisplay(noTransaksi) {
        ...
        $.ajax({
            url: '<?php echo e(route("billing-get-order-detail")); ?>',
            method: 'POST',
            data: {
                _token: token,
    ...
    function openCustomerDisplay() {
        if (custDisplayWindow && !custDisplayWindow.closed) {
            ...
            const url = "<?php echo e(route('fpenjualan-custdisplay-new')); ?>";
    ...
    function sendReceiptEmail() {
        ...
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: '<?php echo e(route("billing-send-receipt-email")); ?>',
                    method: 'POST',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        NoTransaksi: noFaktur,
    ...
    function onBayarFromDetail() {
        ...
        function doSubmit(doCheckout) {
            ...
            fetch('<?php echo e(route("billing-pay-order-detail")); ?>', {
                method: 'POST',
                headers: {
                    ...
                    if (res.snap_token) {
                        ...
                        window.snap.pay(res.snap_token, {
                            onSuccess: function (result) {
                                fetch('<?php echo e(route("billing-midtrans-success")); ?>', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                            ...
                            onClose: function () {
                                btn.disabled = false;
                                btn.innerHTML = oldHtml;
                                fetch('<?php echo e(route("billing-midtrans-cancel")); ?>', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    function submitTambahDurasi() {
        ...
        fetch('<?php echo e(route("billing-store-tambah-durasi")); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': token
                ...
                <form id="frmPilihPaketNew" autocomplete="off">
                    ...
                    <div class="pp-row pp-row-2">
                        <div class="pp-field">
                            ...
                            <select class="pp-input" id="ppJenisPaket" name="JenisPaket" onchange="onJenisPaketChange(this.value)">
                                ...
                                <?php if(isset($jenisLangganan) && count($jenisLangganan) > 0): ?>
                                    <?php $__currentLoopData = $jenisLangganan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php
                        ...
                        <div class="pp-field">
                            ...
                            <select class="pp-input mt-1" id="ppKodePelanggan" name="KodePelanggan" style="margin-top:6px;">
                                ...
                                <?php $__currentLoopData = $pelanggan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($plg->KodePelanggan); ?>"><?php echo e($plg->NamaPelanggan); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                    </div>

                    <?php if(isset($company[0]) && $company[0]->JenisUsaha == 'Apotek'): ?>
                    <!-- Row: Apotek -->
                    <div class="pp-row">
                        <div class="pp-field">
                    ...
                    <div class="pp-row">
                        <div class="pp-field">
                            ...
                            <select class="pp-input" id="ppKodeSales" name="KodeSales" onchange="validateForm()" <?php echo e((Auth::user()->KodeSales != '') ? 'disabled' : ''); ?>>
                                ...
                                <?php $__currentLoopData = $sales; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sls): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($sls->KodeSales); ?>" <?php echo e($sls->KodeSales == Auth::user()->KodeSales ? 'selected' : ''); ?>><?php echo e($sls->NamaSales); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                    ...
                    <div class="fnb-selection-area" style="margin-top: 15px;">
                        ...
                        <div class="fnb-list-container" id="ppFnbMenuList">
                            <?php $__currentLoopData = $itemmaster; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="fnb-item" data-name="<?php echo e(strtolower($item->NamaItem)); ?>">
                                    <img src="<?php echo e($item->Gambar ? (str_starts_with($item->Gambar, 'http') ? $item->Gambar : asset('assets/img/item/' . $item->Gambar)) : 'data:image/svg+xml;base64,'); ?>" 
                                         class="fnb-item-img" 
                                         onerror="this.src='data:image/svg+xml;base64,'">
                                    <div class="fnb-item-info">
                                        <div class="fnb-item-name"><?php echo e($item->NamaItem); ?></div>
                                        <div class="fnb-item-price">Rp <?php echo e(number_format($item->HargaJual)); ?></div>
                                        <div class="fnb-item-stock">Stok: <?php echo e($item->Stock); ?></div>
                                    </div>
                                    <div class="fnb-qty-ctrl">
                                        <button type="button" class="fnb-qty-btn" onclick='addPpFnbToCart(<?php echo json_encode($item, 15, 512) ?>, -1)'>-</button>
                                        <span class="fnb-qty-val pp-fnb-qty-val" data-kode="<?php echo e($item->KodeItem); ?>">0</span>
                                        <button type="button" class="fnb-qty-btn" onclick='addPpFnbToCart(<?php echo json_encode($item, 15, 512) ?>, 1)'>+</button>
                                    </div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <select class="pp-input" id="ppMetodePembayaran" name="MetodePembayaran" onchange="calculateTotal()">
                                    <?php $__currentLoopData = $metodepembayaran; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($mp->id); ?>" 
                                            data-tipe="<?php echo e($mp->TipePembayaran); ?>"
                                            data-percent="<?php echo e($mp->BiayaAdminPercent ?? 0); ?>"
                                            data-rupiah="<?php echo e($mp->BiayaAdminRupiah ?? 0); ?>"
                                            data-verifikasi="<?php echo e($mp->MetodeVerifikasi); ?>">
                                            <?php echo e($mp->NamaMetodePembayaran); ?>
            ...
            <div class="pp-body">
                ...
                <div class="fnb-selection-area">
                    ...
                    <div class="fnb-list-container" id="fnbMenuList" style="max-height: 400px;">
                        <?php $__currentLoopData = $itemmaster; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(in_array($item->TypeItem, [1,2,3,5])): ?>
                            <div class="fnb-item" data-name="<?php echo e(strtolower($item->NamaItem)); ?>">
                                ...
                                <div class="fnb-qty-ctrl">
                                    <button type="button" class="fnb-qty-btn" onclick='addFnbToCart(<?php echo json_encode($item, 15, 512) ?>, -1)'>-</button>
                                    <span class="fnb-qty-val fnb-qty-val" data-kode="<?php echo e($item->KodeItem); ?>">0</span>
                                    <button type="button" class="fnb-qty-btn" onclick='addFnbToCart(<?php echo json_encode($item, 15, 512) ?>, 1)'>+</button>
                                </div>
                            </div>
                            <?php endif; ?>
                ...
                <div id="fnbPaymentSection" style="display: none; background: #e3f2fd; border: 1px solid #bbdefb; border-radius: 8px; padding: 15px; margin-top: 15px;">
                    ...
                    <div class="detail-calc-row">
                        <span>PPN (<span id="fnbCalcPpnPersen"><?php echo e($company[0]->PPN ?? 0); ?></span>%):</span>
                        <span id="fnbCalcPpnRp">Rp 0</span>
                    </div>
                    <div class="detail-calc-row">
                        <span>Biaya Layanan (<span id="fnbCalcServicePersen"><?php echo e($company[0]->ServiceCharge ?? 0); ?></span>%):</span>
        ...
        <div class="pp-modal" style="max-width: 500px;">
            ...
            <div class="pp-body">
                ...
                <div id="tdPaymentSection" style="display: none; background: #f1f8e9; border: 1px solid #c5e1a5; border-radius: 8px; padding: 15px; margin-top: 15px;">
                    ...
                    <div class="detail-calc-row">
                        <span>PPN (<span id="tdCalcPpnPersen"><?php echo e($company[0]->PPN ?? 0); ?></span>%):</span>
                        <span id="tdCalcPpnRp">Rp 0</span>
                    </div>
                    <div class="detail-calc-row">
                        <span>Biaya Layanan (<span id="tdCalcServicePersen"><?php echo e($company[0]->ServiceCharge ?? 0); ?></span>%):</span>
        ...
        <div class="receipt-container">
            <div class="receipt-paper" id="receiptContent">
                ...
                <div class="receipt-info">
                    ...
                    <div class="receipt-info-row">
                        <span>Kasir: <span id="rcptCashier"><?php echo e(Auth::user()->name); ?></span></span>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                        <div style="grid-column:span 2;">
                            ...
                            <select id="mdMetodePembayaran" class="pp-input" style="width:100%;" onchange="onDetailMetodeChange()">
                                <?php $__currentLoopData = $metodepembayaran; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($mp->id); ?>"
                                        data-nama="<?php echo e($mp->NamaMetodePembayaran); ?>"
                                        data-percent="<?php echo e($mp->BiayaAdminPercent ?? 0); ?>"
                                        data-rupiah="<?php echo e($mp->BiayaAdminRupiah ?? 0); ?>">
                                        <?php echo e($mp->NamaMetodePembayaran); ?>
    ...
    <script>
    // ===== DATA INJECTIONS =====
    var dataPaketAll = <?php echo json_encode($paket); ?>;
    var dataPelangganAll = <?php echo json_encode($pelanggan); ?>; // Data Member Injected
    var dataCustomerMemberships = <?php echo json_encode($customerMemberships ?? []); ?>; // Active Memberships
    var dataGrupPelanggan = <?php echo json_encode($gruppelanggan); ?>; // Member Group
    var confCompany = <?php echo json_encode(count($company) > 0 ? $company[0] : null); ?>;
            ...
            <div style="flex:1; overflow:hidden; padding:20px; display:flex; gap:20px;">
                ...
                <div style="flex:1; min-width:0; display:flex; flex-direction:column; border-right:1px solid #f0f0f0; padding-right:10px;">
                    ...
                    <div style="padding:15px; display:grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap:15px; overflow-y:auto;" id="fnbQuickGrid">
                        <?php $__currentLoopData = $itemmaster; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php $isHabis = ($item->Stock ?? 0) <= 0; ?>
                            <div class="fnb-card" 
                                 onclick="<?php echo e($isHabis ? "swal('Stok Habis', 'Menu tidak tersedia.', 'warning')" : 'addJualFnbToCart(' . json_encode($item) . ')'); ?>" 
                                 style="background:#fff; border:1px solid #eee; border-radius:12px; padding:12px; cursor:pointer; transition:all 0.2s; display:flex; flex-direction:column; align-items:center; text-align:center; box-shadow:0 2px 6px rgba(0,0,0,0.06); position:relative; <?php echo e($isHabis ? 'opacity:0.6; filter:grayscale(0.9);' : ''); ?>">
                                ...
                                <div style="font-weight:700; font-size:0.9rem; color:#333; height:40px; overflow:hidden; line-height:1.2;"><?php echo e($item->NamaItem); ?></div>
                                <div style="font-weight:800; color:#e65100; margin-top:5px; font-size:1rem;">Rp <?php echo e(number_format($item->HargaJual)); ?></div>
                                <div style="font-size:0.75rem; color:<?php echo e($isHabis ? '#d32f2f' : '#888'); ?>; margin-top:2px;">Stok: <?php echo e(number_format($item->Stock ?? 0)); ?></div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
                        ...
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                            ...
                            <div>
                                ...
                                <select id="jualFnbMetode" onchange="calculateJualFnbTotal()" style="width:100%; padding:6px; border:1px solid #ccc; border-radius:6px; font-size:0.85rem; margin-bottom:8px;">
                                    <?php $__currentLoopData = $metodepembayaran; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($mp->id); ?>" data-percent="<?php echo e($mp->AdminFeePercent ?? 0); ?>" data-rupiah="<?php echo e($mp->AdminFeeRupiah ?? 0); ?>" data-tipe="<?php echo e($mp->TipePembayaran ?? ''); ?>" data-nama="<?php echo e($mp->NamaMetodePembayaran); ?>"><?php echo e($mp->NamaMetodePembayaran); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
    ...
    function searchJualFnbItems(query) {
        ...
        $.ajax({
            url: "<?php echo e(route('itemmaster-GetStockPerWhs')); ?>",
            method: 'POST',
            data: { 
                Scan: query, 
                Active: 'Y', 
                TipeItemIN: '1,2,3,5',
                KodeGudang: "<?php echo e($company[0]->GudangPoS ?? 'GDG01'); ?>"
            },
    ...
    function submitJualFnb() {
        ...
        swal({
            ...
            fetch('<?php echo e(route("billing-jual-fnb-standalone")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
<?php /**PATH D:\OneDrive\My Project Aplikasi\pos.dstechsmart.com\resources\views/Transaksi/Penjualan/PoS/billing_new.blade.php ENDPATH**/ ?>