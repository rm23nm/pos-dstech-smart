...
<head>
  ...
  <title>Monitor Peracikan Obat - xPOS</title>
  ...
  <style>
    :root {
      --bg-color: <?php echo e(($company && $company->TypeKitchenBackgraund == 'Color') ? ($company->KitchenBackgraund ?? '#1e1e2f') : '#1e1e2f'); ?>;
      --card-bg: #27293d;
      --text-main: #ffffff;
      --text-muted: #9a9a9a;
    ...
    html, body {
      ...
      <?php if($company && $company->TypeKitchenBackgraund == 'Image' && !empty($company->KitchenBackgraund)): ?>
      background-image: url('<?php echo e($company->KitchenBackgraund); ?>');
...
<body>
  ...
  <!-- HEADER -->
  <header>
    <div id="company-name">Monitor Peracikan Obat</div>
    <div class="d-flex align-items-center gap-3">
        <div class="text-muted small">Server Time: <?php echo e(\Carbon\Carbon::now('Asia/Jakarta')->format('H:i:s')); ?></div>
        <div id="clock">--:--:--</div>
    </div>
  </header>
  ...
  <div class="filter-section flex-wrap">
      <div class="d-flex align-items-center gap-2">
          ...
          <select id="categoryFilter" class="form-select" style="width: 200px;">
              ...
              <?php $__currentLoopData = $jenisItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jenis): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($jenis->KodeJenis); ?>"><?php echo e($jenis->NamaJenis); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </select>
      </div>
      <div class="d-flex align-items-center gap-2">
          <label for="tableFilter" class="form-label mb-0">Antrean:</label>
          <select id="tableFilter" class="form-select" style="width: 200px;">
              <option value="">Semua Antrean</option>
              <?php $__currentLoopData = $tables; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $table): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($table->id); ?>"><?php echo e($table->QueueNumber); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </select>
      </div>
      <div class="d-flex align-items-center gap-2">
          <label for="dateFilter" class="form-label mb-0">Tanggal:</label>
          <input type="date" id="dateFilter" class="form-select" style="width: 160px;" value="<?php echo e(date('Y-m-d')); ?>">
      </div>
  ...
  <script>
    ...
    function fetchKitchenData() {
        $.ajax({
            url: "<?php echo e(route('infoperacikan-data')); ?>",
            method: "POST",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                RecordOwnerID: "<?php echo e(Auth::user()->RecordOwnerID); ?>",
        ...
        items.forEach(item => {
            if (!groupedItems[item.NoTransaksi]) {
                groupedItems[item.NoTransaksi] = {
                    ...
                    QueueNumber: item.QueueNumber || 'Take Away',
        ...
        Object.values(groupedItems).forEach(group => {
            ...
            if (jamMulaiDate && jamMulaiDate > nowTime) {
                ...
                prepBanner = `<div class="prep-time-banner urgent">
                    ...
                    <div class="prep-countdown" style="font-size:1rem;">Antrean sudah aktif</div>
                </div>`;
            }
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="kitchen-card fade-in" style="border-left-color: ${group.OrderStatus == 2 ? '#22c55e' : (group.OrderStatus == 1 ? '#fbbf24' : '#1d8cf8')}">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                             <div class="table-name">Antrean: ${group.QueueNumber}</div>
                        ...
                        <div class="order-items">
                            ${group.details.map(item => `
                                <div class="item-row d-flex justify-content-between align-items-center">
                                    ...
                                    <button class="btn ${item.isCompleted ? 'btn-outline-secondary' : 'btn-success'} btn-sm px-3" onclick="markItemDone('${item.NoTransaksi}', '${item.NoUrut}', ${item.isCompleted})" ${item.isCompleted ? 'disabled' : ''}>
    ...
    function updateOrderStatus(noTrx, status) {
        $.ajax({
            url: "<?php echo e(route('infoperacikan-updatestatus')); ?>",
            method: "POST",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                NoTransaksi: noTrx,
    ...
    function markItemDone(noTrx, lineNo) {
        ...
        $.ajax({
            url: "<?php echo e(route('infoperacikan-markdone')); ?>",
            method: "POST",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                RecordOwnerID: "<?php echo e(Auth::user()->RecordOwnerID); ?>",
    ...
    function printOrder(noTrx, btn) {
        const url = "<?php echo e(route('infokitchen-print')); ?>?NoTransaksi=" + noTrx;
    ...
    function recallOrder(noTrx) {
        $.ajax({
            url: "<?php echo e(route('recall-order')); ?>",
            method: "POST",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                NoTransaksi: noTrx
...
<?php /**PATH D:\OneDrive\My Project Aplikasi\pos.dstechsmart.com\resources\views/Transaksi/Penjualan/InfoPeracikan.blade.php ENDPATH**/ ?>