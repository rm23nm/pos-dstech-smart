							...
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tbody>
										...
										<tr>
											<td><?php echo e($u->name); ?><br><small class="text-muted"><?php echo e($u->email); ?></small></td>
											<td>
												<input type="text" class="form-control input-format-angka mb-1" name="gaji[<?php echo e($u->id); ?>][GajiPokok]" value="<?php echo e(number_format($u->GajiPokok ?? 0, 0, '', ',')); ?>" style="min-width:120px;">
											</td>
											<td>
												<input type="text" class="form-control input-format-angka mb-1" name="gaji[<?php echo e($u->id); ?>][TarifLemburPerJam]" value="<?php echo e(number_format($u->TarifLemburPerJam ?? 0, 0, '', ',')); ?>" style="min-width:120px;">
											</td>
											<td>
												<input type="text" class="form-control input-format-angka mb-1" name="gaji[<?php echo e($u->id); ?>][TarifDendaPerMenit]" value="<?php echo e(number_format($u->TarifDendaPerMenit ?? 0, 0, '', ',')); ?>" style="min-width:120px;">
											</td>
											<td>
												<select class="form-control mb-1" name="gaji[<?php echo e($u->id); ?>][KodeAkunGaji]" style="min-width:180px;">
													<option value="">-- Pilih Akun Beban Gaji --</option>
													<?php $__currentLoopData = $rekeningBeban; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rek): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
														<option value="<?php echo e($rek->KodeRekening); ?>" <?php echo e(($u->KodeAkunGaji == $rek->KodeRekening) ? 'selected' : ''); ?>><?php echo e($rek->KodeRekening); ?> - <?php echo e($rek->NamaRekening); ?></option>
													<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
												</select>
											</td>
											<td class="text-center">
												<button type="button" class="btn btn-sm btn-info btn-komponen" data-id="<?php echo e($u->id); ?>" data-name="<?php echo e($u->name); ?>">
...
<script>
	jQuery(document).ready(function() {
		...
		jQuery('#formMasterGaji').on('submit', function(e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					jQuery.ajax({
						url: '<?php echo e(route("master-gaji-store")); ?>',
						type: 'POST',
						data: formData,
		...
		jQuery('.btn-komponen').on('click', function() {
			...
			Swal.showLoading();
			jQuery.ajax({
				url: '<?php echo e(url("master-gaji/komponen")); ?>/' + uid,
				type: 'GET',
				success: function(res) {
					Swal.close();
					if(res && res.length > 0) {
		...
		jQuery('#formKomponen').on('submit', function(e) {
			...
			let formData = jQuery(this).serialize();
			jQuery.ajax({
				url: '<?php echo e(route("master-gaji-komponen-store")); ?>',
				type: 'POST',
				data: formData,
				success: function(res) {
					if(res.success) {
...
<?php echo $__env->make('parts.header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\OneDrive\My Project Aplikasi\pos.dstechsmart.com\resources\views/hrd/MasterGaji.blade.php ENDPATH**/ ?>