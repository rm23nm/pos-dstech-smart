                                ...
                                <div class="icons d-flex align-items-center">
                                    <div class="me-3 mb-1">
                                        <select class="form-select form-select-sm" id="LoketIDPanggil" title="Pilih Loket Anda">
                                            <?php $__currentLoopData = $lokets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($l->id); ?>"><?php echo e($l->NamaLoket); ?></option>
                                    ...
                                    <table class="table table-bordered table-hover" id="tableAppointment">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo e(date('d-m-Y', strtotime($a->TanggalDaftar))); ?></td>
                                                <td><?php echo e($a->NoRM); ?></td>
                                                <td><?php echo e($a->NamaPasien); ?></td>
                                                ...
                                                <td>
                                                    ...
                                                    <?php else: ?>
                                                        <span class="badge bg-secondary"><?php echo e($a->Status); ?></span>
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <button class="btn btn-sm btn-success" onclick="panggilPoli(<?php echo e($a->id); ?>)" title="Panggil ke Poli"><i class="fas fa-volume-up"></i></button>
                                                    <?php if($a->Status == 'Diperiksa'): ?>
                                                        <button class="btn btn-sm btn-danger" onclick="ulangiPanggilPoli(<?php echo e($a->id); ?>)" title="Ulangi Panggilan"><i class="fas fa-redo"></i></button>
                                                    <?php endif; ?>
                                                    <button class="btn btn-sm btn-info" onclick="editStatus(<?php echo e($a->id); ?>, '<?php echo e($a->Status); ?>')" title="Ubah Status"><i class="fas fa-tasks"></i></button>
                                                    <a href="<?php echo e(route('klinik-appointments.destroy', $a->id)); ?>" class="btn btn-sm btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus antrean ini?')" title="Hapus"><i class="fas fa-trash"></i></a>
                                                </td>
                                            </tr>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </tbody>
...
<div class="modal fade" id="modalAppointment" tabindex="-1" aria-labelledby="modalAppointmentLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="formAppointment" method="POST" action="<?php echo e(route('klinik-appointments.store')); ?>">
                <?php echo csrf_field(); ?>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="date" name="TanggalDaftar" id="TanggalDaftar" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="PatientID" id="PatientID" class="form-control select2" required style="width: 100%;">
                            ...
                            <?php $__currentLoopData = $patients; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($p->id); ?>"><?php echo e($p->NoRM); ?> - <?php echo e($p->NamaPasien); ?> <?php if($p->NIK): ?> [NIK: <?php echo e($p->NIK); ?>] <?php endif; ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="DoctorID" id="DoctorID" class="form-control select2" required style="width: 100%;">
                            ...
                            <?php $__currentLoopData = $doctors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($d->id); ?>"><?php echo e($d->NamaDokter); ?> (<?php echo e($d->Spesialisasi); ?>)</option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div class="mb-3">
...
window.$ = window.jQuery;
    $(document).ready(function() {
        ...
        $(document).on('click', '#btnPanggilKiosk', function() {
            ...
            $.ajax({
                url: "<?php echo e(route('klinik-kiosk.panggil')); ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '#btnUlangiPanggil', function() {
            ...
            $.ajax({
                url: "<?php echo e(route('klinik-kiosk.ulangi')); ?>",
                type: "POST",
                data: { 
                    _token: "<?php echo e(csrf_token()); ?>",
                    loket_id: loketId
        ...
        $('#PatientID').on('change', function() {
            ...
            if (patientId) {
                $.ajax({
                    url: "<?php echo e(url('klinik-appointments/get-patient-bpjs')); ?>/" + patientId,
                    type: "GET",
        ...
        $('#PoliID').on('change', function() {
            ...
            if (poliId) {
                $.ajax({
                    url: "<?php echo e(url('klinik-appointments/doctors')); ?>/" + poliId,
                    type: "GET",
                    success: function(data) {
    ...
    function tambahAppointment() {
        ...
        $('#formAppointment').attr('action', '<?php echo e(route('klinik-appointments.store')); ?>');
        ...
        $('#TanggalDaftar').val('<?php echo e(date('Y-m-d')); ?>');
    }

    function editStatus(id, currentStatus) {
        $('#formStatus').attr('action', '<?php echo e(url('klinik-appointments/update')); ?>/' + id);
    ...
    function ulangiPanggilPoli(id) {
        $.ajax({
            url: "<?php echo e(route('klinik-appointments.ulangi')); ?>",
            type: "POST",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                id: id
    ...
    $(document).on('submit', '#formQuickPatient', function(e) {
        ...
        $.ajax({
            url: "<?php echo e(route('klinik-patients.store')); ?>",
            type: "POST",
...
<?php echo $__env->make('parts.header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\OneDrive\My Project Aplikasi\pos.dstechsmart.com\resources\views/klinik/appointments/index.blade.php ENDPATH**/ ?>